...
//        String regex = "^1\\d{10}$";
        String regex = "(\\d*-\\d*|\\d*-\\d*-\\d*|(\\+\\d+)?(\\d{3,4}-?)?\\d{7,8}$)";
    ...
    public static boolean checkPassWord(String passWord) {
        String regex = "^(?![A-Z]+$)(?![a-z]+$)(?!\\d+$)(?![\\W_]+$)\\S{8,18}$";/*(?!^\d+$)(?!^[a-zA-Z]+$)[0-9a-zA-Z]{4,23}*/
        if (passWord == null) {
            return false;
        }
    ...
    //校验pin码
    public static boolean checkPinCode(String smsCode) {
        String regex = "\\d{4}$";
        if (smsCode == null) {
            return false;
        }
