package com.dpad.telematicsclientapp.weiget.toast;
...
public class StyleableToast extends LinearLayout {
    ...
    private ImageView toastIm;
    private int gravity;
    private Toast toast;
    private LinearLayout rootLayout;
    private int TOAST_ERROR = 2;
    private int TOAST_OK = 1;
    private int type;
    ...
    private void inflateToastLayout() {
        ...
        toastIm = v.findViewById(R.id.toast_im);

        if (style > 0) {
            typedArray = getContext().obtainStyledAttributes(style, R.styleable.StyleableToast);
        }
        if (this.type == TOAST_OK) {
            toastIm.setVisibility(VISIBLE);
            toastIm.setImageResource(R.mipmap.toast_ok);

        } else if (this.type == TOAST_ERROR) {
            toastIm.setVisibility(VISIBLE);
            toastIm.setImageResource(R.mipmap.toast_error);
        } else {
            toastIm.setVisibility(GONE);
        }
