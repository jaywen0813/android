...
public class BaseTitleView extends RelativeLayout {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        TypedArray typeArray = context.obtainStyledAttributes(attrs, R.styleable.BaseTitleView);
    ...
    private void initLeftTextView(Context context, TypedArray typeArray) {
        int leftText = typeArray.getResourceId(R.styleable.BaseTitleView_left_text, 0);
        CharSequence charSequence = leftText > 0 ?
                typeArray.getResources().getText(leftText) : typeArray.getString(R.styleable.BaseTitleView_left_text);

        LayoutParams params = initLayoutParams();
        params.setMargins(UIUtils.dip2px(30), 0, 0, 0);
        /*init TextView*/
        mLeftBackTextTv = createTextView(context, R.id.tv_left_text, charSequence, params);
        setTextViewDrawable(typeArray, R.styleable.BaseTitleView_left_text_drawable_left, R.styleable
                .BaseTitleView_left_text_drawable_right, mLeftBackTextTv);

        params.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
        params.addRule(RelativeLayout.CENTER_VERTICAL);

        float textSize = getDimensionPixelSize(typeArray, R.styleable.BaseTitleView_small_text_size, defaultSmallTextSize);
    ...
    private void initLeftImageView(Context context, TypedArray typeArray) {
        int leftImgAttr = typeArray.getResourceId(R.styleable.BaseTitleView_left_image, 0);

        if (leftImgAttr == 0) {
            return;
        }
        LayoutParams params = initLayoutParams();
        int dp5 = mResources.getDimensionPixelSize(R.dimen.dimen_value_dp5);
    ...
    private void initTitleView(Context context, TypedArray typeArray) {
        int leftText = typeArray.getResourceId(R.styleable.BaseTitleView_title_name, 0);
        CharSequence charSequence = leftText > 0 ?
                typeArray.getResources().getText(leftText) : typeArray.getString(R.styleable.BaseTitleView_title_name);

        LayoutParams params = initLayoutParams();
        /*init TextView*/
        mTitleTv = createTextView(context, R.id.tv_title_name, charSequence, params);
        setTextViewDrawable(typeArray, R.styleable.BaseTitleView_title_drawable_left, R.styleable
                .BaseTitleView_title_drawable_right, mTitleTv);


        float textSize = getDimensionPixelSize(typeArray, R.styleable.BaseTitleView_title_text_size, defaultTitleSize);
        mTitleTv.setTextSize(TypedValue.COMPLEX_UNIT_PX, textSize);
        //防止title 文字过长
        mTitleTv.setLines(1);
        mTitleTv.setEllipsize(TextUtils.TruncateAt.END);
        LayoutParams lp = (LayoutParams) mTitleTv.getLayoutParams();
        int dp35 = mResources.getDimensionPixelSize(R.dimen.dimen_value_dp35);
        lp.setMargins(dp35, 0, dp35, 0);
        mTitleTv.setLayoutParams(lp);
        mTitleTv.setTextColor(getTextColorFromAttr(typeArray));

        int gravity = typeArray.getInt(R.styleable.BaseTitleView_title_gravity, 0);
        if (gravity > 0) {
            if (gravity == Gravity.LEFT) {
                params.addRule(RelativeLayout.RIGHT_OF, mLeftBackImageTv == null ? mLeftBackTextTv.getId() :
                        mLeftBackImageTv.getId());
            }
        } else { //default:center
    ...
    private void initRightTextView(Context context, TypedArray typeArray) {
        int leftText = typeArray.getResourceId(R.styleable.BaseTitleView_right_text, 0);
        CharSequence charSequence = leftText > 0 ?
                typeArray.getResources().getText(leftText) : typeArray.getString(R.styleable.BaseTitleView_right_text);

        LayoutParams params = initLayoutParams();
        /*init TextView*/
        mRightTextTv = createTextView(context, R.id.tv_right_text, charSequence, params);
        setTextViewDrawable(typeArray, R.styleable.BaseTitleView_right_text_drawable_left, R.styleable
                .BaseTitleView_right_text_drawable_right, mRightTextTv);
    ...
    private void initRightAnotherTextView(Context context, TypedArray typeArray) {
        int leftText = typeArray.getResourceId(R.styleable.BaseTitleView_right_text_two, 0);
        CharSequence charSequence = leftText > 0 ?
                typeArray.getResources().getText(leftText) : typeArray.getString(R.styleable.BaseTitleView_right_text_two);

        LayoutParams params = initLayoutParams();
        /*init TextView*/
        mRightTextTwoTv = createTextView(context, R.id.tv_right_text_two, charSequence, params);
        setTextViewDrawable(typeArray, R.styleable.BaseTitleView_right_text_two_drawable_left, R.styleable
                .BaseTitleView_right_text_two_drawable_right, mRightTextTwoTv);
    ...
    private void initRightImageView(Context context, TypedArray typeArray) {
        int rightImgAttr = typeArray.getResourceId(R.styleable.BaseTitleView_right_image, 0);

        if (rightImgAttr == 0) {
            return;
        }
        LayoutParams params = initLayoutParams();
        int dp5 = mResources.getDimensionPixelOffset(R.dimen.dimen_value_dp5);
        params.setMargins(0, 0, dp5, 0);
    ...
    private void initRightAnotherImageView(Context context, TypedArray typeArray) {
        int rightImgAttr = typeArray.getResourceId(R.styleable.BaseTitleView_right_image_two, 0);

        if (rightImgAttr == 0) {
            return;
        }
        LayoutParams params = initImageLayoutParams();
     ...
     * @return LayoutParams
     */
    private LayoutParams initImageLayoutParams() {
        return new LayoutParams(UIUtils.dip2px(25), UIUtils.dip2px(25));
    }
     ...
     */
    private void setTextViewDrawable(TypedArray typeArray, int leftDrawableStyleable, int rightDrawableStyleable,
                                     TextView textView) {
    ...
    private float getDimensionPixelSize(TypedArray typeArray, int stylable, int defaultSize) {
        ...
        return sizeStyleable > 0 ? typeArray.getResources().getDimensionPixelSize(sizeStyleable) : typeArray
                .getDimensionPixelSize(stylable, (int) getPiselSizeBySp(defaultSize));
    }
    ...
    private int getTextColorFromAttr(TypedArray typeArray) {
        int textColorStyleable = typeArray.getResourceId(R.styleable.BaseTitleView_title_text_color, 0);
        if (textColorStyleable > 0) {
            return typeArray.getResources().getColor(textColorStyleable);
        } else {
            return typeArray.getColor(R.styleable.BaseTitleView_title_text_color, textColor);
        }
    ...
    public void setTitle(int rsId) {
        setTitle(mResources.getString(rsId));
    }
