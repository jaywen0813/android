...
import static com.dpad.telematicsclientapp.weiget.idcardcamera.camera.CameraActivity.rotateBitmap;
...
public class ImageUtils {
 ...
 /**
     ...
     * @return {@code true}: 成功<br>{@code false}: 失败
     */
    public static boolean saveThird(Bitmap src, String filePath, CompressFormat format,boolean isThird) {
        return saveThird(src, FileUtils.getFileByPath(filePath), format, false,isThird);
    }
    ...
    public static boolean save(Bitmap src, File file, CompressFormat format, boolean recycle) {
        ...
        src = rotateBitmap(src, -90f);
        System.out.println(src.getWidth() + ", " + src.getHeight());
        OutputStream os = null;
        boolean ret = false;
        try {
            os = new BufferedOutputStream(new FileOutputStream(file));
            ret = src.compress(format, 75, os);
     ...
     * @return {@code true}: 成功<br>{@code false}: 失败
     */
    public static boolean saveThird(Bitmap src, File file, CompressFormat format, boolean recycle,boolean isThird) {
        if (isEmptyBitmap(src) || !FileUtils.createOrExistsFile(file)) {
            return false;
        }
        if (!isThird) {
